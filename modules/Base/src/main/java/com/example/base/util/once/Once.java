...
public class Once {
     ...
     * <p>
     * Equivalent of calling {@code beenDone(int scope, String tag, CountChecker numberOfTimes)} with scope of {@code THIS_APP_INSTALL} and a numberOfTimes of {@code Amount.Amount.moreThan(0)}.
     * </p>
     *
    ...
    public static boolean beenDone(@Scope int scope, String tag, CountChecker numberOfTimes) {
        ...
        if (scope == THIS_APP_INSTALL) {
            ...
            for (String tagFromList : sessionList) {
                if (tagFromList != null && tagFromList.equals(tag)) {
                    counter++;
                }
            }
    ...
    public static boolean beenDone(long timeSpanInMillis, String tag, CountChecker numberOfTimes) {
        ...
        for (Long seenDate : tagSeenDates) {
            ...
            if (Math.abs(seenDate - new Date().getTime()) < timeSpanInMillis) {
                counter++;
            }
...
//            }
    ...
    /**
     * Checks if a tag has been marked done range (startTimeSpanInMillis,endTimeSpanInMillis] milliseconds
     * a specific number of times.
     *
     * @param startTimeSpanInMillis Start Time Span
     * @param endTimeSpanInMillis   End Time Span
     * @param tag                   A string identifier unique to the operation.
     * @param numberOfTimes         Requirement for how many times the operation must have be done.
     * @return {@code true} if the operation associated with {@code tag} has been marked done
     * within the last X milliseconds.
     */
    public static boolean beenDone(long startTimeSpanInMillis, long endTimeSpanInMillis, String tag, CountChecker numberOfTimes) {
        List<Long> tagSeenDates = tagLastSeenMap.get(tag);
        ...
        int counter = 0;
        for (Long seenDate : tagSeenDates) {
            if (seenDate > startTimeSpanInMillis && seenDate <= endTimeSpanInMillis) {
                counter++;
            }
        }
    ...
    public static boolean todayBeenDone(String tag) {
        List<Long> tagSeenDates = tagLastSeenMap.get(tag);
        ...
        int counter = 0;
        for (Long seenDate : tagSeenDates) {
            if (seenDate > getTodayStartTimeMillis() && seenDate <= getTodayEndTimeMillis()) {
                return true;
            }
        }
    ...
    public static boolean todayBeenDone(String tag, CountChecker numberOfTimes) {
        List<Long> tagSeenDates = tagLastSeenMap.get(tag);
     ...
     * @param tag A string identifier unique to the operation.
     */
    public static void clearAndMarkDone(String tag) {
        markDone(tag, true);
    }
     ...
     * @param tag A string identifier unique to the operation.
     */
    public static void markDone(String tag, boolean clearOldData) {
        if (clearOldData) {
            clearDone(tag);
        }
